import { url as urlbase } from "../settings/settings.js";
import { msg as MESSAGE } from "../settings/settings.js";
import { id_buttonName as idButtonName } from "../settings/settings.js";
...
import { reportLimitsOfSaucenao, unsupportedData } from "./reportToOwner.js";
...
var parseSauceNao = function (response, bot, editMsg) {
  ...
  if (!results.length) {
    track(editMsg.origFrom, "sauce_not_found", { url: editMsg.url });

    return [getGoogleSearch(MESSAGE.zeroResult, editMsg.url)];
  }
  ...
  var restOfIds = arraysInCommon(idbaseArray, Object.keys(data));
  ...
  if (restOfIds.length) {
    ...
    if (restOfIds.length > 5) restOfIds.splice(5); //keep only 5 items
    ...
    for (var j = 0; j < restOfIds.length; j++) {
      ...
      if (restOfIds[j] == "pawoo_id") id = data["pawoo_user_acct"] + "/" + id;
      else if (restOfIds[j] == "bcy_id")
        id = data["bcy_type"] + "/detail/" + data["member_link_id"] + "/" + id;
      else if (restOfIds[j] == "url") id = data["url"];

      if (j == 0) buttonName = "View on " + buttonName;
    ...
    buttons = buttonsGridify(bList);
  } else {
    preview = true;
    unsupportedData(element, bot);
...
var createDetailedText = (header, data, showThumbnail) => {
  textarray = [
    ...
    (data.title ? "\n<b>" + data.title + "</b>" : "") +
      " " +
      (data.member_name || data.creator
        ? "<b>by:</b> " + (data.member_name || data.creator)
        : ""),
  ...
  txt = textarray.join("");
  if (txt.length < 2) txt = " (no metadata)";
  return txt;
};
