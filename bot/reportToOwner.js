...
const receiver_id = require("../settings/settings.js").private.adminId;
const reportToOwnerSwitch = require("../settings/settings.js").reportToOwnerSwitch;
module.exports = {
    reportLimitsOfSaucenao: (header, bot) => {
        ...
        if (longLimit> 10 || shortLimit > 10)
            return;
        const text = "⏰ <b>Saucenao Req Limit: (Remain/Limit)</b>\n" +
    ...
    reportFile: (file, bot, force) => {
        ...
        for (let i = 0; i < receiver_id.length; i++) {
            ...
            bot.telegram.sendPhoto(receiver_id[i], file, {
                caption: null,
                disable_notification: !reportToOwnerSwitch.reportFile.notify
            })
            .catch( err => {
              if(err.error_code && err.error_code==400){
                bot.telegram.sendSticker(receiver_id[i], file)
                .catch( err => {
                  if(err.error_code && err.error_code==400)
                    bot.telegram.sendDocument(receiver_id[i], file)
